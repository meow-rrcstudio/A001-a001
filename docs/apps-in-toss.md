...
토큰 길은 뚫렸지만, 브라우저가 **다른 출처에서 부르는 것 자체**를 막습니다.
`Access-Control-Allow-Origin` 을 내줘야 하는데, **미니앱이 어떤 출처로
뜨는지 아직 모릅니다** (`.ait` 번들이라 `null` 일 수도, 토스 전용 스킴일
수도 있습니다).

⚠️ 모른다고 `*` 로 열지 않았습니다. 아무 사이트나 우리 API 를 부를 수 있게
   됩니다 — 토큰이 있어야 하니 당장 새는 것은 없지만, 열어둘 까닭도 없습니다.
   미니앱을 한 번 띄워 **실제 `Origin` 헤더를 본 뒤** 그 값만 허용합니다.
...
0. ✅ **서버가 토큰을 받게 하기** — 콘솔 없이도 할 수 있어서 먼저 했습니다
   (§3). 이게 없으면 미니앱을 만들어도 API 를 하나도 못 부릅니다.
1. 8/21 — 웹 오픈 (카카오페이)
2. 콘솔에서 앱 등록 → `appName` 확정, 상품(SKU) 3개 등록
3. 미니앱 프런트 껍데기 + 토스 로그인 → 우리 세션 잇기
   (여기서 미니앱의 실제 `Origin` 을 보고 CORS 를 그 값만 열어줍니다)
