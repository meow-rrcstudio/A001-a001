...
const RULES: Rule[] = [
  ...
  },
  {
    category: "투자",
    topicKey: "money",
    // ⚠️ 당첨·로또·복권·추첨이 함께 있어야 합니다. 이 낱말들이 없을 때는
    //    "로또 당첨될까"가 기타(normal)로 흘러서, 맞히듯 말하지 말라는
    //    지침이 통째로 빠졌습니다 — 정작 가장 맞혀달라고 조르는 물음입니다.
    //    ("청약"은 원래 있어서 "청약 당첨"만 우연히 걸리고 있었습니다)
    pattern:
      /주식|코인|비트코인|이더리움|가상\s*자산|선물\s*투자|펀드|종목|매수|매도|손절|익절|수익률|시세|환율|재테크|부동산|청약|투자|당첨|로또|복권|추첨/,
  },
  {
    category: "연애",
    topicKey: "love",
 ...
 * 지금 누가 나를 해치고 있는 자리.
 *
 * ⚠️ HEAVY_CARE 와 갈라 둡니다. 무거운 마음은 "곁에 있겠다"고 앉는 말인데,
 *    맞고 있는 사람에게 그 말을 하면 그 자리에 더 앉아 있으라는 말이 됩니다.
 *    여기서는 안전한 자리를 찾는 것이 먼저입니다.
 *
 * ⚠️ 시안(26_선행)에는 자·타해 쪽만 있어서 이 문구는 같은 결로 지어 넣은
 *    것입니다. 확정 전이니 바꿔도 됩니다.
 *
 *    두 브랜치가 각자 써 두었던 것을 여기서 하나로 합쳤습니다. 짧은
 *    쪽("안전한 곳을 먼저 찾으세요")을 버린 것은 그 문장만 샨티의 말투가
 *    아니어서입니다 — 다른 안내는 모두 "~다냥"으로 말을 겁니다.
 */
